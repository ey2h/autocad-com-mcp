...
# ── 出图不在这里做 ─────────────────────────────────────────────────────
#
# 曾尝试用 COM 出图（Layout.ConfigName + PlotType + PlotToFile），结论是**不如**
# CurtainWallAI 插件里那条出图管线，原因：
#
#   1. 官方 SetWindowToPlot 文档说窗口坐标的单位由 PaperUnits 决定 —— 不是 WCS。
#      实测 PlotType=acWindow + SetWindowToPlot(WCS)：GetWindowToPlot 能精确读回、
#      PlotToFile 返回 True，但出图**全白**（2560x1440 里非白像素 0 个）。
#   2. PlotType=acView(3) / acLayout(5) 在这台机器上直接设不上；acLimits(2) 也全白；
#      acExtents(1) 能出，但那是**全图**（91 张图跨 9 万单位），单张只是像素点。
#   3. 唯一能用的是 acDisplay(0) + 先 ZoomWindow 设视图 —— 但它打的是**屏幕视口**，
#      屏幕是 16:9 而图框内框是 700x574，于是内框塞进 16:9 后**四周留白，
#      像素与坐标的对应关系就不准了**（出图必须能 1:1 反推坐标）。
#
# 插件那条管线用 2.3 秒/张出图 + 按内容裁剪，像素<->坐标误差 <=1 像素，已经验证过
# 91 帧 74 秒 0 失败。**能跑通且更准的东西不要为了架构好看去重写。**
#
# 本服务保留不可替代的那部分：**看和指**（zoom / select / highlight / query）。
# ─────────────────────────────────────────────────────────────────────
